...
// MARK: - Buffer.Protocol Conformance (Slab.Inline, sparse / bitmap-domain count)

/// `Buffer.Slab.Inline` is a `Buffer.Protocol` capability conformer.
///
...
/// `Buffer.Slab.Inline+Operations.swift`), overriding the protocol's element-domain
...
extension Buffer.Slab.Inline: Buffer.`Protocol` where S: ~Copyable {
    ...
    // `Element` is the enclosing `Buffer<Element>` element type.
