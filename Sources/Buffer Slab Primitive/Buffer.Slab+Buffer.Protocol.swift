...
/// `Index<Element>.Count` at the `count` witness in `Buffer.Slab+Operations.swift`
/// (one occupied slot IS one live element). The slab supplies its own `isEmpty`,
/// though the unconstrained default (`count == .zero`) would now also fire.
///
