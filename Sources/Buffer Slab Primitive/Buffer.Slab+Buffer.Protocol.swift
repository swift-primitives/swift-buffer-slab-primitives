...
// MARK: - Buffer.Protocol Conformance (Slab, sparse / bitmap-domain count)

/// `Buffer.Slab` is a `Buffer.Protocol` capability conformer.
///
...
/// `Buffer.Slab+Operations.swift`), overriding the protocol's element-domain
/// default. `count` equals the live-element cardinality (`occupancy`); the slab
/// supplies its own `isEmpty` (the protocol's default `isEmpty` is constrained to
/// the element-domain default `Count`).
///
...
extension Buffer.Slab: Buffer.`Protocol` where S: ~Copyable {
    ...
    // both associated types explicitly. `Count` is the bitmap (slot) domain;
    // `Element` is the substrate's element type.
